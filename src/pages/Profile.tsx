...
import { collection, query, where, orderBy, onSnapshot, doc, updateDoc, setDoc, limit, getDocs, startAfter, QueryDocumentSnapshot } from '../lib/firebase';
import { ref, uploadBytes, getDownloadURL } from '../lib/firebase';
import { useUser, useClerk } from '@clerk/clerk-react';
import { formatPrice, cn } from '../lib/utils';
import { Package, Calendar, Clock, CheckCircle, Truck, XCircle, User, LogOut, Shield, Star, Gift, ArrowUpRight, ArrowDownRight, Heart, ShoppingCart, Camera, Upload, Zap, ShieldCheck, Droplets, Hammer, Settings, ChevronDown, ListFilter, Info, Bell, Home, ArrowRight, MessageCircle, Download } from 'lucide-react';
...
export default function Profile() {
  ...
  const isAdmin = useMemo(() => user?.primaryEmailAddress?.emailAddress === 'chakrabortytamanash@gmail.com', [user]);
  ...
  useEffect(() => {
    ...
    const unsubPoints = getUserPoints(user.id, (points) => {
      setLoyaltyPoints(points);
    });

    const unsubTransactions = getLoyaltyTransactions(user.id, (txs) => {
      setTransactions(txs);
    });
  ...
  const uploadProfilePic = async () => {
    ...
    try {
      ...
      // Update Clerk Profile (if necessary, handled by Clerk usually but keeping placeholder)
      try {
        await user.update({
          // image updates in Clerk require different API, skipping for now, relying on Firebase
        });
      } catch (e) {}
  ...
  const loadMoreBookings = async () => {
    if (!user || !lastBookingDoc || loadingMore) return;
    setLoadingMore(true);
    try {
      const q = query(
        collection(db, 'bookings'),
  ...
  const getBookingStatusStyles = (status: string, date: string) => {
    ...
    if (isPastDate(date)) return 'text-gray-500 bg-white/5 border-text-primary/5';
  ...
  return (
    <div className="min-h-screen bg-bg-primary pb-32 pt-32">
       <div className="container-custom space-y-16">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
             <div className="flex items-center gap-6">
                ...
                <div>
                   <h1 className="text-3xl font-black text-white uppercase italic tracking-tighter">{user?.firstName || user?.fullName || 'RIDER'}</h1>
                   <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">{user?.primaryEmailAddress?.emailAddress}</p>
                </div>
             </div>
              ...
              <div className="space-y-16">
                ...
                <section className="space-y-12">
                  <div className="flex items-center justify-between">
                    ...
                    <Link to="/blog" className="px-6 py-3 rounded-full border border-text-primary/10 text-[9px] font-black uppercase tracking-widest hover:border-brand-orange hover:text-brand-orange transition-all">Full Archives</Link>
                  </div>
                  <BlogSection />
                </section>
              </div>
          ...
          orders.length === 0 ? (
            <div className="glass p-24 text-center rounded-[3rem] border border-text-primary/10">
            ...
            orders.map(order => (
              <motion.div 
                ...
                className="glass rounded-[2.5rem] border border-text-primary/10 overflow-hidden"
              >
                <div className="p-8 border-b border-text-primary/5 flex flex-wrap justify-between items-center gap-6">
                  ...
                  <div className="flex items-center space-x-8">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <button 
                        onClick={() => generateInvoice(order, { name: displayName, email: user?.primaryEmailAddress?.emailAddress || '', phone })}
                        className="p-2 bg-white/5 hover:bg-brand-orange text-white/50 hover:text-white rounded-xl transition-all border border-text-primary/10 hover:border-brand-orange shadow-lg flex items-center justify-center group/btn"
                        title="Download Bill"
                      >
                ...
                <div className="p-8 bg-white/5">
                  <div className="flex flex-wrap gap-4">
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex items-center space-x-3 bg-black/20 p-2 pr-4 rounded-xl border border-text-primary/5">
          ...
          <div className="space-y-8">
            <div className="flex justify-between items-center px-4">
              ...
              {bookings.length > 0 && (
                <div className="text-xs font-bold text-gray-500 uppercase tracking-widest bg-white/5 py-2 px-4 rounded-full border border-text-primary/10">
                  ...
                  return (
                    <motion.div 
                      ...
                      className={cn(
                        ...
                        isPast || isCancelled ? "border-text-primary/5 opacity-80" : "border-text-primary/10"
                      )}
                    >
                      <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="flex items-center space-x-6 w-full md:w-auto">
                          <div className={cn(
                            ...
                            isPast ? "bg-white/5 border-text-primary/10 text-gray-600" :
                        ...
                        <div className="flex flex-wrap items-center justify-between md:justify-end gap-6 w-full md:w-auto">
                          ...
                          <button 
                            ...
                            className="bg-white/5 hover:bg-white/10 px-4 py-2 rounded-xl text-xs font-bold transition-all border border-text-primary/10 flex items-center space-x-2"
                          >
            ...
            {hasMoreBookings && (
              <div className="text-center pt-8">
                <button 
                  ...
                  className="bg-white/5 hover:bg-white/10 px-8 py-4 rounded-full text-sm font-bold border border-text-primary/10 transition-all flex items-center space-x-3 mx-auto"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {wishlistItems.map(item => (
                <motion.div 
                  ...
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="glass rounded-3xl overflow-hidden border border-text-primary/10 group"
                >
                  <div className="aspect-video relative overflow-hidden">
                    <img 
                      ...
                      loading="lazy"
                    />
                    <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border border-text-primary/10">
                      {item.type}
                    </div>
                    <button 
                      onClick={() => {
                      ...
                      className="absolute top-4 right-4 p-2 bg-black/60 backdrop-blur-md rounded-full hover:bg-red-500/20 text-white hover:text-red-500 transition-colors border border-text-primary/10"
                    >
          ...
          <div className="space-y-8">
            {/* Points Summary Card */}
            <div className="glass p-12 rounded-[3rem] border border-text-primary/10 bg-gradient-to-br from-brand-orange/20 to-transparent relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1 space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={reward.points} className="glass p-6 rounded-3xl border border-text-primary/10 flex items-center justify-between group">
                      <div>
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="glass rounded-[2.5rem] border border-text-primary/10 overflow-hidden">
          ...
          <div className="space-y-6">
            ...
            <div className="glass rounded-[2.5rem] border border-text-primary/10 overflow-hidden bg-bg-primary">
              <div className="p-8">
                <p className="text-gray-500 mb-8">Stay updated with your service progress and order status.</p>
                <div className="bg-white/5 rounded-3xl p-6 border border-text-primary/5">
          ...
          <div className="max-w-2xl mx-auto">
            <div className="glass p-8 md:p-12 rounded-[3rem] border border-text-primary/10">
              ...
              <div className="mb-10 flex flex-col items-center">
                <div className="relative group">
                  <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-brand-orange/20 p-1 relative">
                    <img 
                      src={previewUrl || user?.imageUrl || `https://ui-avatars.com/api/?name=${user?.fullName}&background=random`} 
                      alt="Profile Preview" 
              ...
              <form onSubmit={handleSaveProfile} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/5 border border-text-primary/10 rounded-xl p-4 focus:outline-none focus:border-brand-orange transition-colors"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    disabled 
                    className="w-full bg-white/5 border border-text-primary/10 rounded-xl p-4 text-gray-400 cursor-not-allowed"
                  />
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-white/5 border border-text-primary/10 rounded-xl p-4 focus:outline-none focus:border-brand-orange transition-colors min-h-[120px]"
                  />
                </div>
                <button 
                  type="submit"
        ...
        {selectedBooking && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg glass rounded-[2.5rem] border border-text-primary/10 p-8 overflow-hidden"
            >
              <button 
                onClick={() => setSelectedBooking(null)}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 p-4 rounded-2xl border border-text-primary/5">
                ...
                <div className="bg-white/5 p-6 rounded-2xl border border-text-primary/5">
