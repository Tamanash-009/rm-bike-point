...
import { collection, getDocs, addDoc, serverTimestamp } from '../lib/firebase';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <section className="mb-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-8 gap-4">
          {loading ? (
            [...Array(8)].map((_, i) => (
              <div key={i} className="p-6 rounded-[2rem] border border-text-primary/5 bg-white/5 space-y-3">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-text-primary/10 rounded-full py-4 pl-12 pr-6 focus:outline-none focus:border-brand-orange transition-colors"
            />
          </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  category === cat 
                    ...
                    : "bg-white/5 border-text-primary/10 text-gray-400 hover:border-white/30"
                )}
              >
                {cat}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="space-y-2">
            ...
            <select 
              ...
              className="w-full bg-white/5 border border-text-primary/10 rounded-2xl py-3 px-6 focus:outline-none focus:border-brand-orange transition-colors appearance-none"
            >
              {categories.map(cat => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProducts.map((product) => (
            <motion.div
              ...
              className="glass rounded-3xl overflow-hidden group border border-text-primary/10 hover:border-brand-orange/30 transition-all"
            >
              <Link to={`/shop/${product.id}`} className="block aspect-square overflow-hidden relative">
                ...
                <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-brand-orange border border-text-primary/10">
                  {product.category}
                </div>
              </Link>
                ...
                {product.bikeModels && product.bikeModels.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {product.bikeModels.slice(0, 3).map(model => (
                      <span key={model} className="text-[9px] bg-white/5 px-2 py-0.5 rounded-full border border-text-primary/5 text-gray-400">
                        {model}
                      </span>
                    ))}
