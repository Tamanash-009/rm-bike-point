...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const unsubPoints = getUserPoints(user.id, (pts) => {
        setUserPoints(pts);
        setInitialLoading(false);
      });
      
      import('../lib/firebase').then(({ doc, getDoc }) => {
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-bg-primary pb-32">
        ...
        <div className="sticky top-0 z-50 bg-black/60 backdrop-blur-xl border-b border-text-primary/5 px-4 py-4 md:py-6 flex items-center justify-between">
          <button 
            onClick={() => navigate(-1)}
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 md:py-24 text-center">
          ...
          <div className="max-w-md mx-auto space-y-4 mb-24">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Link 
                to="/services" 
                className="bg-white/5 border border-text-primary/10 hover:bg-white/10 text-white p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all group"
              >
                  ...
                  popularProducts.map((p, i) => (
                    ...
                    >
                      <Link to={p.type === 'product' ? `/shop/${p.id}` : `/bikes/${p.id}`} className="group">
                        <div className="aspect-square rounded-3xl overflow-hidden border border-text-primary/10 bg-white/5 mb-4 group-hover:border-brand-orange/50 transition-colors">
                          <img 
                            src={p.imageUrl} 
                            alt={p.name}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pb-32">
      {/* Mobile Top Header */}
      <div className="lg:hidden sticky top-0 z-50 bg-black/60 backdrop-blur-xl -mx-4 sm:-mx-6 px-4 py-4 mb-8 flex items-center justify-between border-b border-text-primary/5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="space-y-6">
            {items.map((item) => (
              <motion.div
                ...
                className="glass p-6 rounded-[2rem] border border-text-primary/10 flex flex-col sm:flex-row items-center gap-6"
              >
                <img
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center bg-white/5 rounded-2xl border border-text-primary/10 p-1">
                    <button
        ...
        <div className="space-y-6">
          {initialLoading ? (
            <div className="glass p-8 rounded-[2.5rem] border border-text-primary/10 space-y-4">
            ...
            user && userPoints > 0 && (
              <div className="glass p-8 rounded-[2.5rem] border border-text-primary/10">
          ...
          <div className="glass p-8 rounded-[2.5rem] border border-text-primary/10 sticky top-24">
            ...
            {!userAddress && user && !initialLoading && (
              <div className="mb-6 p-5 bg-white/5 border border-text-primary/10 rounded-2xl border-l-brand-orange border-l-2">
            ...
            <div className="mt-8 pt-6 border-t border-text-primary/5 flex flex-col items-center gap-4">
