...
import { collection, query, orderBy, onSnapshot, serverTimestamp, doc, updateDoc, addDoc, deleteDoc, getDocs, writeBatch } from '../lib/firebase';
...
export default function AdminChat() {
  ...
  return (
    <div className="glass rounded-[3rem] border border-text-primary/10 overflow-hidden h-[700px] flex">
      {/* Sidebar */}
      <div className="w-1/3 border-r border-text-primary/10 flex flex-col">
        <div className="p-6 border-b border-text-primary/10">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-text-primary/10 rounded-xl py-3 pl-12 pr-4 text-sm focus:outline-none focus:border-brand-orange transition-colors"
            />
          </div>
        </div>
          ...
          {rooms.length === 0 ? (
            ...
            rooms.map((room) => (
              <button
                ...
                className={cn(
                  "w-full p-6 flex items-center space-x-4 transition-all border-b border-text-primary/5",
      ...
      <div className="flex-1 flex flex-col bg-black/20">
        {selectedRoom ? (
          <>
            {/* Chat Header */}
            <div className="p-6 border-b border-text-primary/10 flex items-center justify-between bg-white/5">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
              {messages.map((msg) => (
                ...
                >
                  ...
                  <div className={cn(
                    "px-6 py-4 rounded-3xl text-sm leading-relaxed shadow-xl border border-text-primary/5",
                    msg.isAdmin 
            ...
            {/* Input */}
            <form onSubmit={handleSendMessage} className="p-8 border-t border-text-primary/10">
              <div className="relative">
                <input
                  type="text"
                  value={inputText}
                  ...
                  className="w-full bg-white/5 border border-text-primary/10 rounded-2xl py-5 pl-8 pr-16 focus:outline-none focus:border-brand-orange transition-colors"
                />
                <button 
                  type="submit"
